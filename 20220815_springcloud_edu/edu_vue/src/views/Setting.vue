...
export default {
    ...
    methods: {
        ...
        // 上传头像
        myUpload(content) {
            let form = new FormData(); // 必须将表单中的数据进行封装后才能发送，否则传不过去
            //form.append("file", content.file.name);
			form.append("file", content.file);
            console.log(content);
            console.log(form);
            this.axios.post("/user/upload/",form)
            .then(result => {  
                // 上传成功后，返回的图片fileId
        ...
        //更新个人信息
        updateInfo(){
            return this.axios.get("http://localhost:80/user/updateUser/",{
                params:{
        ...
        //修改密码
        updatePwd(){
            if(this.oldPwd == this.user.content.password){ // 原密码比对
                if(this.newPwd1 == this.newPwd2){ //两次密码比对
                    return this.axios.get("http://localhost:80/user/updatePassword/",{
                        params:{
                            userid:this.user.content.id,
                            newPwd:this.newPwd2
                        }
